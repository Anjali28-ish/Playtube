...
import { FaMicrophoneAlt, FaChartBar, FaVideo, FaPlusCircle, FaTachometerAlt } from "react-icons/fa";
    ...
    <div className="bg-[#0f0f0f] text-white min-h-screen flex flex-col">
      <header className="h-14 px-4 sm:px-6 flex items-center justify-between border-b border-gray-800 bg-[#0f0f0f] shadow-md">
        <div
          ...
          onClick={() => navigate("/")}
        >
          <SiYoutubestudio className="text-orange-500 h-7 w-7" />
          <h1 className="text-lg sm:text-xl font-bold tracking-wide">
            <span className="text-[#ffffff]">PTStudio</span>
          </h1>
        </div>
        <div className='flex items-center gap-3 sm:gap-4'>
            
         <button className="bg-[#272727]
           px-3 sm:px-4 py-1 lg:rounded-lg rounded-full hover:bg-[#161414]  active:scale-95 transition
          cursor-pointer text-sm hidden md:flex items-center justify-center gap-1"
          onClick={()=>navigate("/create")}><FaPlusCircle />
         <span>Create</span></button>
         <img src={channelData?.avatar} alt="" className='w-8 h-8 sm:w-9
         sm:h-9 rounded-full border border-gray-600 object-cover
         hover:scale-105 transition' onClick={()=>setOpen(!open)}/>
...
function SidebarItem({icon,text, onClick,active, setActive }){
  ...
  className={`flex items-center gap-2 text-sm py-2 rounded-lg transition-all ${
    isActive
